          ...
          <div className="space-y-2 h-full flex flex-col">
            ...
            <div className="bg-muted/30 rounded-lg p-2 flex-shrink-0">
              ...
              <div className="text-xs space-y-1">
                <div className="font-mono bg-muted p-1 rounded break-all text-xs">
                  {config.BACKEND_URL}
                </div>
                <div className="text-muted-foreground text-xs">
                  {connectionStatus === 'success' && '✓ Backend responding'}
                  {connectionStatus === 'failed' && '✗ Cannot reach backend'}
                  {connectionStatus === 'unknown' && 'Test connectivity'}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 flex-1 overflow-hidden">
                  ...
                  <div key={index} className="border rounded-lg p-2 space-y-1">
                    ...
                    {entry.error && (
                      <div className="text-xs text-destructive">
                        Error: {entry.error.length > 30 ? `${entry.error.substring(0, 30)}...` : entry.error}
                      </div>
                    )}
                  </div>
                ))}
                {debugEntries.length > 2 && (
                  <div className="text-xs text-center text-muted-foreground py-1">
                    ... and {debugEntries.length - 2} more
                  </div>
                )}
              </div>
            )}
